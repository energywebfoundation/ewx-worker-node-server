...
export interface Runtime {
  ...
  /**
   * One-time initialization. For NR this runs RED.init + RED.start and returns
   * the http.Server so graceful shutdown can close it. For n8n this spawns the
   * child process and waits until the REST API is reachable, returning null
   * (the child process is tracked internally, not as an http.Server).
   * Called during main bootstrap.
   */
  start: (app: Express) => Promise<Server | null>;
